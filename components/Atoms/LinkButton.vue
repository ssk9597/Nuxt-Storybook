<template>
  <div>
    <nuxt-link :to="link" :class="classes">
      {{ btnName }}
      <span v-if="isIcon">
        <fa :class="{ icon: isIcon }" :icon="[iconType, iconName]" />
      </span>
    </nuxt-link>
  </div>
</template>
...
export default {
  ...
  props: {
    ...
    btnType: {
      ...
      default: 'normal',
      validator: function (value) {
        return ['normal', 'rounded-corners', 'round'].indexOf(value) !== -1;
      },
    },
    btnSize: {
      type: String,
    ...
    iconType: {
      ...
      required: false,
      validator: function (value) {
        return ['fas', 'fab', 'far'].indexOf(value) !== -1;
      },
    },
    iconName: {
      type: String,
  ...
  computed: {
    classes() {
      return {
        'el-btn': true,
        [`${this.btnColor}`]: true,
        [`el-btn-type-${this.btnType}`]: true,
        [`el-btn-size-${this.btnSize}`]: true,
        'icon-btn': this.isIcon,
      };
    },
  },
};
